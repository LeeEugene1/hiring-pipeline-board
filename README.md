...
- 이름 부분 검색과 직무 선택 필터의 복합 적용 및 초기화
- 카드 선택으로 여는 지원자 상세 패널과 접근성 포커스 관리
- 보드 형태 초기 로딩, 조회 오류·재시도와 두 종류의 빈 상태
...
- 같은 카드를 연속 이동하면 이전 요청을 중단하고 마지막으로 선택한 단계만 화면과 Mock API에 반영합니다.

DnD 대신 명시적 액션을 선택한 배경과 트레이드오프는 [DECISIONS.md](./DECISIONS.md#결정-4-명시적-액션-버튼으로-단계를-이동한다)에 기록했습니다.

## 지원자 검색과 필터

- 이름 검색은 대소문자와 앞뒤 공백을 정규화한 뒤 부분 일치로 찾습니다.
- 직무 필터를 함께 선택하면 두 조건을 모두 만족하는 지원자만 표시합니다.
- 검색 결과 수를 전체 지원자 수와 함께 표시하며 초기화 버튼으로 두 조건을 한 번에 해제할 수 있습니다.
- 200건 목록에서 입력 반응성을 유지하도록 검색어는 `useDeferredValue`, 결과와 직무 목록은 `useMemo`로 계산합니다.
...
- 최초 조회 중에는 다섯 단계 보드 형태를 유지하는 스켈레톤을 표시합니다.
- 조회에 실패하면 오류 원인 안내와 중복 요청을 막는 다시 시도 버튼을 제공합니다.
- API 원본 목록이 비면 등록된 지원자 없음 상태를 표시합니다.
- 지원자는 있지만 검색·필터 결과가 비면 별도 안내와 필터 초기화 버튼을 표시합니다.
...
검증이 통과하면 GitHub Actions가 Pull Request에는 Vercel Preview를, `main`에는 Vercel Production을 배포합니다. 포크 Pull Request는 비밀값 보호를 위해 배포하지 않습니다.

테스트는 단계 순서와 개수, 카드 필드, 지원자 중복 여부, 이름 정규화와 부분 검색, 직무 필터 교집합, 필터 초기화, 200건 처리, 상세 패널 정보·외부 클릭·ESC 예외·포커스 복귀, 초기 로딩, 조회 실패와 재시도, 전체·필터 빈 상태, Mock API 조회 연결, 좌우 버튼 경계, 키보드 단계 선택, 응답 전 즉시 이동, 실패 rollback, 다른 후보자 상태 보존과 연속 요청 응답 역전을 검증합니다. 강제 실패 모드는 위의 `localStorage` 설정으로 재현할 수 있습니다.
...
├── features/ # 보드, 검색·필터, 지원자 상세와 단계 이동 기능
