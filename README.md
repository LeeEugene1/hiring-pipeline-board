...
테스트는 단계 순서와 개수, 카드 필드, 지원자 중복 여부, 이름 정규화와 부분 검색, 직무 필터 교집합, 필터 초기화, 200건 처리, Mock API 조회 연결, 좌우 버튼 경계, 키보드 단계 선택, 응답 전 즉시 이동, 실패 rollback, 다른 후보자 상태 보존과 연속 요청 응답 역전을 검증합니다. 강제 실패 모드는 위의 `localStorage` 설정으로 재현할 수 있습니다.
...
├── features/ # 보드, 지원자 카드, 검색·필터와 단계 이동 기능
